...
if (config.common.production) {
  ...
  Sentry.init({
    dsn: config.services.CMS.sentryDSN,
    integrations: [
...
mongoose
  .connect(config.database.mongo.uri, {
    dbName: config.services.CMS.database?.name,
  })
  .catch(err => {
    throw err;
  });
...
app.use(decodeToken(Service.CMS));
...
app.listen(config.services.CMS.port, () => {
  console.log(`CMS service started on port ${config.services.CMS.port}`);
});
